...
class wsVivan extends Controller
{
    ...
    public function empresa()
    {
        ...
        if (count($obj) > 0) {
            ...
            foreach ($obj as $dados) {
                ...
                if ($idEmpresa) {
                    $empresa = (new Empresa())->findById($idEmpresa->id);
                    if ($empresa) {
    ...
    {
        $json = file_get_contents(IP_VIVAN . "/Sistema/sistemaRP/json/servidor_json_bd.php");    // Check in your php.ini so allow_url_fopen is set to on.
        $obj = json_decode($json);
